...
狀態優先順序：`invalid > failed > missing > insufficient_data > partial > stale > delayed > fresh`。
...
## 5. Dataset ID
Dataset ID 必須固定，不因來源或 fallback 改變。範例：
...
規則：優先主要 Proxy；來源失敗才使用 fallback；fallback 必須記錄原因、來源及取得時間；舊資料須標記 `stale` 或 `last_valid`；網頁資料不可直接視為官方即時資料。
...
## 8. 選擇權關鍵位
以下欄位必須分開：`call_wall`、`put_wall`、`gamma_wall`、`gamma_flip`、`max_pain`。無法取得時使用 `null`，不可使用 `0` 代表缺失；每個欄位可個別標記狀態。
