...
def validate_source_manifest(path: Path, source: str, expected_data_date: str) -> tuple[bool, list[str], dict]:
    ...
    # TAIFEX uses t0_trading_date; TWSE uses analysis_date for the same
    # source-data date. Accept the canonical field first and the legacy/source
    # specific field second, while still validating against T0.
    manifest_data_date = manifest.get("t0_trading_date") or manifest.get("analysis_date")
    if manifest_data_date != expected_data_date:
