...
def get(url,retries=3):
    for i in range(retries):
        try:
            r=Request(url,headers={'accept':'application/json','user-agent':'daily-pre-market-analysis/3.0'})
...
def rows(x):
    ...
    if isinstance(x,dict):
        for k in ('data','data1','data8','aaData','result','records'):
...
def val(r,keys):
    return next((num(r[k]) for k in keys if isinstance(r,dict) and k in r and num(r[k]) is not None),None)

def breadth(rs):
    z={'up':0,'down':0,'unchanged':0,'limit_up':0,'limit_down':0};seen=False
    for r in rs:
        c=val(r,['漲跌','漲跌價差','change','Change']);p=val(r,['漲跌幅','漲跌百分比','change_percent','ChangePercent'])
...
def main():
    ...
    idx=pull('twse_openapi_index',f'{OPENAPI}/exchangeReport/MI_INDEX?{urlencode({"response":"json","date":d8,"type":"IND"})}')
    if not rows(idx):idx=pull('twse_web_index',f'{WEB}/MI_INDEX?{urlencode({"response":"json","date":d8,"type":"IND"})}')
    proxy=pull('twse_proxy',f'{PROXY}?date={d8}') or {};pd=proxy.get('data',{}) if isinstance(proxy,dict) else {};ta=pd.get('taiex',{});st=pd.get('market_statistics',{})
    ...
    otc=pull('tpex_openapi_quotes',f'{TPEX}/tpex_mainboard_daily_close_quotes');otct=pull('tpex_openapi_highlight',f'{TPEX}/tpex_mainboard_highlight')
    ir=next((r for r in rows(inst) if isinstance(r,dict) and '投信買賣超股數' in r),{});mr=totalrow(rows(margin));sr=totalrow(rows(sbl));tr=totalrow(rows(turn));ix=next((r for r in rows(idx) if isinstance(r,dict) and any(k in r for k in ('收盤指數','收盤','Close'))),{})
    def first(r,ks,fallback=None):return val(r,ks) if val(r,ks) is not None else val(fallback or {},ks)
    foreign=val(ir,['外陸資買賣超股數(不含外資自營商)']);trust=val(ir,['投信買賣超股數']);dealer=val(ir,['自營商買賣超股數'])
    lt=val(tr,['成交金額(元)','成交金額']);ot=sum(v for v in (val(r,['成交金額','成交金額(元)','成交額']) for r in rows(otct)) if v is not None) or None
    data={'taiex':{'close':first(ix,['收盤指數','收盤','Close'],ta),'open':first(ix,['開盤指數','開盤','Open'],ta),'high':first(ix,['最高指數','最高','High'],ta),'low':first(ix,['最低指數','最低','Low'],ta),'change_points':first(ix,['漲跌點數','漲跌','Change'],ta),'change_percent':first(ix,['漲跌百分比','漲跌幅','ChangePercent'],ta),'turnover_value':val(st,['turnover_value','成交金額(元)','成交金額','turnover'])},'listed_breadth':breadth(rows(pd.get('listed_breadth'))),'otc_breadth':breadth(rows(otc)),'institutional':{'unit':'shares','foreign':foreign,'investment_trust':trust,'dealer':dealer,'total':foreign+trust+dealer if None not in (foreign,trust,dealer) else None},'margin':{'financing_balance':val(mr,['融資餘額(元)','融資餘額']),'financing_change':val(mr,['融資增減(元)','融資增減']),'short_balance':val(mr,['融券餘額(張)','融券餘額']),'short_change':val(mr,['融券增減(張)','融券增減']),'maintenance_ratio':val(mr,['融資維持率(%)','融資維持率'])},'sbl':{'balance':val(sr,['借券餘額','借券餘額(張)']),'short_sale_balance':val(sr,['借券賣出餘額','借券賣出餘額(張)']),'short_sale_change':val(sr,['借券賣出增減','借券賣出增減(張)'])},'turnover':{'listed':lt,'otc':ot,'total':lt+ot if None not in (lt,ot) else None}}
    ...
    if not rows(idx) and not pd:errors.append('twse_core_sources.empty')
    if not otc:errors.append('tpex_quotes.empty')
