...
def normalized_rows(x):
    ...
    data=rows(x)
    headers=None
    for k in ('fields','columns','headers','title'):
...
def main():
    ...
    ir=next((r for r in normalized_rows(inst) if isinstance(r,dict) and ('投信買賣超股數' in r or '投信買賣超股數(股)' in r)),{})
    mr=totalrow(normalized_rows(margin));sr=totalrow(normalized_rows(sbl));tr=totalrow(normalized_rows(turn));ix=next((r for r in normalized_rows(idx) if isinstance(r,dict) and any(k in r for k in ('收盤指數','收盤','Close'))),{})
    def first(keys,primary,fallback):return val(primary,keys) if val(primary,keys) is not None else val(fallback,keys)
    foreign=val(ir,['外陸資買賣超股數(不含外資自營商)','外資及陸資買賣超股數']);trust=val(ir,['投信買賣超股數','投信買賣超股數(股)']);dealer=val(ir,['自營商買賣超股數','自營商買賣超股數(股)'])
    lt=val(tr,['成交金額(元)','成交金額']);ot=sum(v for v in (val(r,['成交金額','成交金額(元)','成交額']) for r in normalized_rows(otct)) if v is not None) or None
    listed=breadth(normalized_rows(pd.get('listed_breadth')));otcb=breadth(normalized_rows(otc))
    data={'taiex':{'close':first(['收盤指數','收盤','Close'],ix,ta),'open':first(['開盤指數','開盤','Open'],ix,ta),'high':first(['最高指數','最高','High'],ix,ta),'low':first(['最低指數','最低','Low'],ix,ta),'change_points':first(['漲跌點數','漲跌','Change'],ix,ta),'change_percent':first(['漲跌百分比','漲跌幅','ChangePercent'],ix,ta),'turnover_value':val(st,['turnover_value','成交金額(元)','成交金額','turnover'])},'listed_breadth':listed,'otc_breadth':otcb,'institutional':{'unit':'shares','foreign':foreign,'investment_trust':trust,'dealer':dealer,'total':foreign+trust+dealer if None not in (foreign,trust,dealer) else None},'margin':{'financing_balance':val(mr,['融資餘額(元)','融資餘額']),'financing_change':val(mr,['融資增減(元)','融資增減']),'short_balance':val(mr,['融券餘額(張)','融券餘額']),'short_change':val(mr,['融券增減(張)','融券增減']),'maintenance_ratio':val(mr,['融資維持率(%)','融資維持率'])},'sbl':{'balance':val(sr,['借券餘額','借券餘額(張)']),'short_sale_balance':val(sr,['借券賣出餘額','借券賣出餘額(張)']),'short_sale_change':val(sr,['借券賣出增減','借券賣出增減(張)'])},'turnover':{'listed':lt,'otc':ot,'total':lt+ot if None not in (lt,ot) else None}}
    ...
    if not normalized_rows(otc):errors.append('tpex_quotes.empty')
